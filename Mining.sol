...
contract BurnSwap is Configurable {
    ...
    function __BurnSwap_init(address governor, address from_, uint factor_, address to_, address mine_) public initializer {
        __Governable_init_unchained(governor);
        __BurnSwap_init_unchained(from_, factor_, to_, mine_);
    }
    
    function __BurnSwap_init_unchained(address from_, uint factor_, address to_, address mine_) public governance {
        from    = from_;
        factor  = factor_;
        to      = to_;
        mine    = mine_;
    ...
    function swap(uint vol) virtual external {
        IERC20(from).safeTransferFrom(msg.sender, BurnAddress, vol);
        uint v = vol.mul(factor).div(1e18);
        IERC20(to).safeTransferFrom(mine, msg.sender, v);
        emit Swap(msg.sender, vol, v);
    }
    event Swap(address indexed sender, uint in_, uint out);
    
    function swapAndSend(uint vol, uint toChainId, address sendTo) virtual external payable {
        IERC20(from).safeTransferFrom(msg.sender, BurnAddress, vol);
        uint v = vol.mul(factor).div(1e18);
        IERC20(to).transferFrom(mine, address(this), v);
        IMappingToken(to).send{value: msg.value}(toChainId, sendTo, v);
        emit Swap(msg.sender, vol, v);
    }
}

interface IMappingToken {
    function send(uint toChainId, address to, uint volume) external payable returns (uint nonce);
}
